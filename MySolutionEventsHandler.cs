...
namespace SokoolTools.VsTools
{
	...
	internal class MySolutionEventsHandler : IVsSelectionEvents, IVsSolutionEvents, IDisposable
	{
		...
		/// </returns>
		/// <param name="pHierOld">
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy" /> interface of the project 
		/// hierarchy for the previous selection.
		/// </param>
		/// <param name="itemidOld">
		/// Identifier of the project item for previous selection. For valid <paramref name="itemidOld" /> values, see 
		/// VSITEMID.
		/// </param>
		/// <param name="pMisOld">
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.IVsMultiItemSelect" /> interface to access 
		/// a previous multiple selection.
		/// </param>
		/// <param name="pScOld">
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.ISelectionContainer" /> interface to 
		/// access Properties window data for the previous selection.
		/// </param>
		/// <param name="pHierNew">
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy" /> interface of the project 
		/// hierarchy for the current selection.
		/// </param>
		/// <param name="itemidNew">
		/// Identifier of the project item for the current selection. For valid <paramref name="itemidNew" /> values, 
		/// see VSITEMID.
		/// </param>
		/// <param name="pMisNew">
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.IVsMultiItemSelect" /> interface providing 
		/// access to objects used for updating the properties window.
		/// </param>
		/// <param name="pScNew">
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.ISelectionContainer" /> interface for the 
		/// current selection.
		/// </param>
		//--------------------------------------------------------------------------------------------------------------
		...
		/// DWORD value representing a particular entry in the array of element values associated with the selection 
		/// context. For valid <paramref name="elementid" /> values, see <see 
		/// cref="T:Microsoft.VisualStudio.VSConstants.VSSELELEMID" />.
		/// </param>
		/// <param name="varValueOld">
		/// VARIANT that contains the previous element value. This parameter contains element-specific data, such as a 
		/// pointer to the <see cref="T:Microsoft.VisualStudio.OLE.Interop.IOleCommandTarget" /> interface if <paramref 
		/// name="elementid" /> is set to SEID_ResultsList or a pointer to the <see 
		/// cref="T:Microsoft.VisualStudio.OLE.Interop.IOleUndoManager" /> interface if <paramref name="elementid" /> 
		/// is set to SEID_UndoManager.
		/// </param>
		/// <param name="varValueNew">
		/// VARIANT that contains the new element value. This parameter contains element-specific data, such as a 
		/// pointer to the IOleCommandTarget interface if <paramref name="elementid" /> is set to SEID_ResultsList or a 
		/// pointer to the IOleUndoManager interface if <paramref name="elementid" /> is set to SEID_UndoManager.
		/// </param>
		//--------------------------------------------------------------------------------------------------------------
		...
		/// DWORD representation of the GUID identifying the command UI context passed in as the rguidCmdUI parameter 
		/// in the call to <see 
		/// cref="M:Microsoft.VisualStudio.Shell.Interop.IVsMonitorSelection.GetCmdUIContextCookie(System.Guid@,System.UInt32@)"/>.
		/// </param>
		/// <param name="fActive">
		/// Flag that is set to true if the command UI context identified by <paramref name="dwCmdUICookie" /> has 
		/// become active and false if it has become inactive.
		/// </param>
		//--------------------------------------------------------------------------------------------------------------
		...
		/// </param>
		/// <param name="fAdded">
		/// true if the project is added to the solution after the solution is opened. false if the project is added to 
		/// the solution while the solution is being opened.
		/// </param>
		//--------------------------------------------------------------------------------------------------------------
		...
		/// true if the project is being removed from the solution before the solution is closed. false if the project 
		/// is being removed from the solution while the solution is being closed.
		/// </param>
		/// <param name="pfCancel">
		/// true if the client vetoed the closing of the project. false if the client approved the closing of the 
		/// project.
		/// </param>
		//--------------------------------------------------------------------------------------------------------------
		...
		/// </param>
		/// <param name="fRemoved">
		/// true if the project was removed from the solution before the solution was closed. false if the project was 
		/// removed from the solution while the solution was being closed.
		/// </param>
		//--------------------------------------------------------------------------------------------------------------
		...
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy" /> interface of the 
		/// placeholder hierarchy for the unloaded project.
		/// </param>
		...
		/// <param name="pStubHierarchy">
		/// Pointer to the <see cref="T:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy" /> interface of the 
		/// placeholder hierarchy for the project being unloaded.
		/// </param>
		//--------------------------------------------------------------------------------------------------------------
